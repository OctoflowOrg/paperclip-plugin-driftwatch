...
const SYSTEM_PROMPT = `You are auditing a set of agent instruction files for a multi-agent pipeline.
...
function parseAuditJson(raw: string, provider: Provider): unknown {
  try {
    return JSON.parse(raw);
  } catch (error) {
    throw new Error(
      `${provider} returned non-JSON audit output: ${
...
function extractOpenAIOutputText(data: unknown): string {
  ...
  for (const item of output) {
    ...
    for (const block of content) {
      ...
      if (
        blockRecord.type === 'refusal' &&
        typeof blockRecord.refusal === 'string'
      ) {
...
async function callAnthropic(
  ...
  const data = (await readJsonResponse(res, 'Anthropic')) as {
...
const plugin = definePlugin({
  async setup(ctx) {
    ...
    // Data: fetch all agents for a company
    ...
    // Data: fetch instruction bundle file list for an agent
    ...
    // Data: fetch a single instruction file's content
    ...
    // Action: run drift analysis across agent files (Anthropic or OpenAI)
    ctx.actions.register('run-analysis', async (params) => {
      const agents = params.agents as AgentPayload[];

      const pluginConfig = await ctx.config.get();
      const provider = (pluginConfig.provider as Provider) ?? 'openai';
      const model = (pluginConfig.model as string) ?? 'gpt-5';

      // Resolve API key: plugin config override → env var fallback
      const keyRef =
        provider === 'openai'
  ...
  async onHealth() {
    return { status: 'ok' as const, message: 'DriftWatch active' };
  },
});
